...
* **Dropdown category styling** — enable **This sheet has categories** and select a category
  ...
  assign its own FontAwesome icon and marker background color in the appearance dialog. Each
  category also has a **Colors** button that maps values from another sheet column to exact
  marker colors; current column values are preloaded and future values can be added manually.
  Mappings can use either a monochromatic ramp based on a color you choose or the standard
  multi-color palette.
...
4. Click a sheet's colored icon to open the appearance dialog, then choose or
   ...
   with its own icon and color. Use the option's **Colors** button to choose another source
   column and configure exact value-to-color mappings. The dialog preloads values currently in
   that column and also accepts additional values that may appear later. Without sign-in, the app
   uses distinct values currently present in the category column. Choose either a monochromatic
   ramp with a custom base color or the standard color palette for the mapping values.
